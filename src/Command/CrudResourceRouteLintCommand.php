...
#[AsCommand(name: 'crud:resource:lint-routes', description: 'Validate Cruding resource routes against the route-token convention.')]
/**
 * Exposes the resource route lint command workflow through the Symfony console.
 */
final class CrudResourceRouteLintCommand extends Command
{
    ...
    {
        $this->addOption('strict-provider', null, InputOption::VALUE_NONE, 'Fail concrete non-fallback routes when no resource provider is registered.');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($this->router->getRouteCollection()->all() as $nameEntity => $route) {
            if (!$this->isResourceControllerRoute($route)) {
                continue;
            }
            ...
            if (!$context instanceof CrudRouteContextDTO) {
        ...
        if ([] !== $errorList) {
            $output->writeln(sprintf('<error>Cruding resource route lint failed: %d checked, %d error(s), %d warning(s).</error>', $checked, count($errorList), count($warningList)));

            return Command::FAILURE;
        }

        $output->writeln(sprintf('<info>Cruding resource route lint passed: %d checked, %d warning(s).</info>', $checked, count($warningList)));
