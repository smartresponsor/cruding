...
final class CrudSurfaceFormFieldBuilder
{
    ...
    public function build(?FormView $form, string $resourcePath): array
    {
        if (null === $form) {
            return [
                ['name' => 'title', 'label' => $this->labelFormatter->humanize($resourcePath), 'type' => 'text', 'required' => false, 'value' => null, 'placeholder' => null, 'helpText' => null, 'validationState' => null, 'errorText' => null, 'options' => []],
        ...
        foreach ($form->children as $name => $child) {
            ...
            $fields[] = [
                'name' => (string) $name,
                'label' => is_string($vars['label'] ?? null) && '' !== $vars['label'] ? $vars['label'] : $this->labelFormatter->humanize((string) $name),
