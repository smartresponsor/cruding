...
/**
 * Resolves tokenized route intent resolver for Cruding request processing.
 */
final readonly class CrudTokenizedRouteIntentResolver
{
    ...
    /**      * Resolves web.      */
    public function resolveWeb(Request $request): ?CrudTokenizedRouteIntentDTO
    {
    ...
    /**      * Resolves api.      */
    public function resolveApi(Request $request): ?CrudTokenizedRouteIntentDTO
    {
        $tokens = $this->requestTokens($request, 'crudPath');
        if ([] === $tokens) {
            return null;
        ...
        if ('GET' === $method) {
            ...
            if ($this->isIdentityToken($identity) && $this->hasValidResourceTokenCount($resourceTokens)) {
                return new CrudTokenizedRouteIntentDTO(
     ...
     */
    private function resolveTokens(array $tokens, string $routeFamily, string $defaultView): ?CrudTokenizedRouteIntentDTO
    {
