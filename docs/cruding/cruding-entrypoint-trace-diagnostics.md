...
- `CrudEntrypointOperationRunner::run()` returns the full `CrudEntrypointResult`.
- `CrudEntrypointOperationRunner::tryRun()` keeps the existing `Response|CrudSurfaceContract|null` API.
