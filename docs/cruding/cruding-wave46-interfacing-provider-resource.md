# Cruding Interfacing view contract

Cruding is limited to CRUD route processing, entity resolution, form handling, mutation guards, API responders, and neutral view payload preparation.

## Browser rendering boundary

Cruding stops at `CrudResourceContract`. Viewing owns the browser render decision and may render through Interfacing or return structured JSON. The canonical runtime fallback chain is:

1. `@Interfacing/<view-or-resource>/index.html.twig`
...
`CrudingExtension` loads the bundle service configuration and prepends the `@Cruding` Twig namespace to `templates/`. The extension exposes a small host-facing configuration view:
