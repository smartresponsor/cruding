...
{
    public function testResolvesLiteralResourceviewActionRoute(): void
    {
        ...
        self::assertSame('compliance', $context->viewPath);
    ...
    public function testResolvesGenericDynamicviewActionRoute(): void
    {
        $resolver = new CrudRouteShapeResolver($this->routerWithRoute('cruding_resource_action', '/{resource}/{subject}/{view}/{action}'));
        $request = Request::create('/alpha/acme-inc/compliance/briefing');
        $request->attributes->set('_route', 'cruding_resource_action');
        $request->attributes->set('resource', 'alpha');
        $request->attributes->set('subject', 'acme-inc');
        $request->attributes->set('view', 'compliance');
    ...
    public function testResolvesItemActionRoute(): void
    {
        ...
        self::assertNotNull($context);
        self::assertSame('alpha', $context->resource);
        self::assertSame('document', $context->viewPath);
    ...
    public function testResolvesViewTokenBeforeItemIdentifier(): void
    {
        $resolver = new CrudRouteShapeResolver($this->routerWithRoute('cruding_view_token_item', '/{resource}/{subject}/{view}/{token}/{item}'));
        $request = Request::create('/alpha/attachment/media/show/acme-inc');
        $request->attributes->set('_route', 'cruding_view_token_item');
        $request->attributes->set('resource', 'alpha');
        $request->attributes->set('subject', 'attachment');
        $request->attributes->set('view', 'media');
    ...
    public function testRouteMapEntryPreservesFullBusinessChainAndTemplateOverride(): void
    {
        ...
        $resolver = new CrudRouteShapeResolver(
            $this->routerWithRoute('cruding_view_token_item', '/{resource}/{subject}/{view}/{token}/{item}'),
        ...
        $request->attributes->set('resource', 'alpha');
        $request->attributes->set('subject', 'attachment');
        $request->attributes->set('view', 'document');
        $request->attributes->set('token', 'show');
